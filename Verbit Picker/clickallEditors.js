...
async function getUserIPAddress() {
  ...
  // Function to search for "waiting for review" text and click on all corresponding links
  async function searchAndClickAll() {
    const links = Array.from(document.querySelectorAll('a'));
    const textToSearch = 'waiting for editing';
    const clickPromises = [];
  
    for (const link of links) {
      const linkText = link.innerText.toLowerCase();
  
      if (linkText.includes(textToSearch)) {
        clickPromises.push(link.click());
      }
    }
  
    if (clickPromises.length === 0) {
      console.log('Job not found: Array ' + textToSearch);
      ...
      try {
        ...
        // Send a message after clicking on the links
        sendTelegramMessage(message);
  
        // Wait for all click promises to resolve before continuing
